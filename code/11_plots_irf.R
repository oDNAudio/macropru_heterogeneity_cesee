...
irf_mpp_act_prepost <- rbind(irf_mpp_act1, irf_mpp_act2, irf_prepost1, irf_prepost2)
irf_mpp_act_prepost$order <- factor(irf_mpp_act_prepost$order, 
                                    levels = unique(irf_mpp_act_prepost$order))

p_mpp_act_prepost <- ggplot(data = irf_mpp_act_prepost, aes(x = Var2-1)) +
  geom_hline(yintercept = 0.25, alpha = 0) +
  geom_ribbon(aes(ymin = value, ymax = hi), colour = "lightblue", fill = "lightblue", alpha = 0.5) +
  geom_line(aes(y = med)) +
  geom_hline(yintercept = 0, lty = 2, col = "red") +
